...
| `fleet-only` | A non-empty fleet with an empty deck. Every stage the document can carry, including a worker the panel cannot see - whose detail names a pipeline step, so that nothing may quietly place it on the track from words upstream wrote about its own blindness. Nothing was recorded at dispatch for any of them and no forge was read, which is the all-absent end of every field version 4 added - so it is also where the rail of unknown length is drawn against the whole stage vocabulary. |
| `rails` | Every rail shape a worker can have, in its working, stopped and finished states: the six-stage validated rail, the five-stage rail that delivers straight to a pull request, the four-stage local one and the three-stage investigation, plus the two ways a rail's length can be unknown - a build worker whose contract nobody recorded, and an investigation standing on a pull request, which is a stage its own rail has no room for. One worker names a pipeline step on a rail that has no validation stage, which is the case the panel refuses to call a mismatch. See docs/decisions/2026-08-31-four-rails.md. |
