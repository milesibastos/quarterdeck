...
- **A halted worker's coarse stage is not in the document.** `Lifecycle` carries
  ...
  blocked or waiting elsewhere shows no position rather than a guessed one. The
  inference needs a validating stage to land on, so a worker stopped on a rail
  that has none - delivery straight to a pull request, or an investigation - can
  never be placed at all. A `lastActiveStage` on `Lifecycle` would close both
  halves.
