...
| Reading a real fleet | Amber | Wired: a configured fleet home, its snapshot command run through the one spawn door, parsed strictly and projected into the fleet and deck parts. Read discipline, refusals and last-known-good are tested against a stub runner with no fleet present, and the panel has been driven against a live fleet by hand. Amber because two of upstream's values have no honest home in the frozen document - `unknown` and a record with no start - and because nothing yet re-reads on anything but a filesystem event in the fleet home. |
