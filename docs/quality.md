...
- **A halted worker's coarse stage is not in the document.** `Lifecycle` carries
  ...
  run inside validation - anchored to the rail's validating stage when it has
  one, and otherwise to the furthest stage the rail and the rest of the
  evidence (an already-open pull request, say) support. A worker that names no
  step at all still shows no position rather than a guessed one, in words. A
  `lastActiveStage` on `Lifecycle` would close that last gap.
