...
- **A pending run of more than a hundred checks understates its progress.** A
  rollup that is not pending means every check reported, so a finished run is
  counted exactly however large it is. Only a pending run past the forge's own
  page bound is approximate, and it is approximate downwards: it says fewer
  checks have finished than have.
