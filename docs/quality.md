...
Last reviewed: 2026-08-31, at the end of the pass that stopped the deck
...
- **A halted worker's coarse stage is not in the document.** `Lifecycle` carries
  ...
  infers a position from `step` when one is named - the pipeline's steps only
  run inside validation - so a worker held or failed inside the pipeline shows
  where it stopped. Three cases get no position: a worker that names no step, a
  worker on a rail with no validating stage for its step to land on (direct-pr,
  research), and an unseen worker. The first two say "position not known" in
  words rather than showing an unlit rail with no explanation, because an unlit
  rail alone reads as a claim that the worker has no position. Anchoring those
  to the rail's working stage was tried and reverted: the step word evidences
  nothing on a rail whose contract skips the pipeline, and it walks a worker
  held after its pull request opened backwards. Adding the last active stage to
  the document, verified against a live snapshot rather than assumed
  unavailable, is `qd-halted-stage-r1`; it closes all three.
